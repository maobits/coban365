 ...
 * @param {string} payload.third_party_note - Código especial de transacción (ej. "debt_to_third_party").
 ...
 * @param {number} [payload.cash_tag] - Saldo resultante esperado en caja después de registrar.
 * @param {string} [payload.reference] - Referencia / nota corta visible.
 * @param {number} [payload.bank_commission] - Comisión bancaria aplicada (signo negativo si es costo).
 * @param {number} [payload.dispersion] - Dispersión aplicada (signo negativo si es costo).
 * @param {number} [payload.total_commission] - Suma de costos (bank_commission + dispersion).
 * @param {number} [payload.accumulated_commission] - Comisión acumulada vigente del tercero ANTES de registrar esta operación.
 *
...
export const createThirdPartyTransaction = async (payload: {
  ...
  type_of_movement?: string; // 🆕 campo soportado
  utility?: number;
  cash_tag?: number;
  reference?: string;
  ...
  accumulated_commission?: number; // 🆕 NUEVO: cuánto llevaba acumulado en comisión antes del pago
}): Promise<any> => {
  try {
    ...
    const response = await fetch(url, {
      ...
      // Mandamos TODO el payload tal cual, incluyendo accumulated_commission si viene
      body: JSON.stringify(payload),
    });
